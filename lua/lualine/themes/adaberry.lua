...
local function get_colors()
  local p = vim.deepcopy(adaberry.palette)
  local config = adaberry.config or {}
  local overrides = config.palette_overrides or {}

  for color, hex in pairs(overrides) do
    p[color] = hex
  end
...
local section_b = { fg = colors.fg1, bg = colors.bg2 }
local section_c = { fg = colors.fg1, bg = colors.bg1 }
local inactive = { fg = colors.fg4, bg = colors.bg0 }

return {
  normal = {
    a = { fg = colors.bg0, bg = colors.fg1, gui = "bold" },
    b = section_b,
    c = section_c,
  },
  insert = {
