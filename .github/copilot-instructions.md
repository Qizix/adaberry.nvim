# Copilot Instructions for adaberry.nvim

## Repository Overview

**adaberry.nvim** is a Neovim colorscheme plugin with treesitter and semantic highlighting support. This is a single-module Lua plugin targeting Neovim 0.8.0+ with extensive customization capabilities.
...
### Core Files
- `lua/adaberry.lua` - Main implementation (1,343 lines) containing all colorscheme logic
- `colors/adaberry.lua` - Simple loader that calls `require("adaberry").load()`
- `README.md` - User documentation with installation and configuration examples
- `doc/adaberry.nvim.txt` - Auto-generated Vim help documentation

### Testing Infrastructure
- `tests/adaberry/adaberry_spec.lua` - Test suite (189 lines) using plenary.nvim
...
#### Manual Loading Test
```bash
nvim --headless -c "set rtp+=$PWD" -c "lua require('adaberry').setup()" -c "colorscheme adaberry" -c "echo 'Colorscheme loaded successfully'" -c "q"
```
...
2. **docs.yml** - Runs on main branch push
   - Auto-generates `doc/adaberry.nvim.txt` from README.md using panvimdoc
...
### Main Module Structure (`lua/adaberry.lua`)
- **Adaberry table**: Main module with `config`, `palette`, `setup()`, and `load()` methods
- **Configuration system**: Deep-mergeable config with extensive defaults
- **Color palette**: Comprehensive adaberry color definitions with override support
- **Highlight groups**: Extensive definitions covering core Vim, treesitter, LSP, and plugin integrations

### Critical Implementation Details
- `setup()` must be called BEFORE `colorscheme adaberry` command
...
1. Edit `lua/adaberry.lua` in the palette section or highlight groups
2. Test with `nvim -c "lua require('adaberry').setup()" -c "colorscheme adaberry"`
...
4. Add tests in `tests/adaberry/adaberry_spec.lua` if needed

### Configuration Changes
1. Update `default_config` table for new options
2. Update type annotations (`@class AdaberryConfig`)
...
### Common Issues
- **"adaberry.nvim: you must use neovim 0.8 or higher"**: Version check failure - use newer Neovim
...
2. Use `:lua print(vim.inspect(require('adaberry').config))` to debug configuration
3. Test color changes with `:lua require('adaberry').load()` for quick iteration
...
### Generated Files
- `doc/adaberry.nvim.txt` - Auto-generated, don't edit manually
